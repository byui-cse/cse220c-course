...
Contact::~Contact(void)
{
    std::cout << "In Contact destructor (address is " << address << ")\n";
}
...
{
    std::cout << "In Account destructor (name is " << name << ")\n";
}
...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        case 1:
        {
            std::for_each(accounts.begin(), accounts.end(), [](auto& it) { std::cout << it.second; });
            break;
        }
        ...
        case 6:
        {
            ...
            getline(std::cin, input);
            int id = stoi(input);
            std::erase_if(accounts, [id](auto &it) { return it.first == id; });
            break;
        }

        case 7:
        {
            float total = std::accumulate(accounts.begin(), accounts.end(), 0, [](auto acc, auto &it) { return acc + it.second.get_balance(); });
        ...
        case 8:
        {
            ...
            std::for_each(accounts.begin(), accounts.end(), [factor](auto &it){ it.second *= factor; });
            break;
        }
